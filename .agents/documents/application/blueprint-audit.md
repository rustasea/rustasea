...
| Contracts realizable in Rust | `tdd.md` §3 trait signatures use stable Rust 1.80+ (`Arc`, `OnceLock`, `async_trait`, `serde`, `jsonwebtoken`, `pgvector`); no `unsafe`, no `any`, no `static mut`; proc-macros via `syn`/`quote` | PASS |
