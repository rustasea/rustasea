...
> **Crates:** `rustavel-cli` · `rustavel-macros` · `rustavel-testing` · `xtask` (`cargo-xtask` bin)
...
|---------|-------------|--------|
| CLI | `cargo rustavel list [--json] [--all]`, typed `Args`/`Flags`, prompts `ask`/`secret`/`confirm`/`choice`/`multiSelect`, helpers `table`/`progressBar`/`spinner`, `Shutdownable` | [cli.md](cli.md) |
