...
> **Method:** Every finding re-verified against the source tree on `origin/master` (`7807ae4`) — evidence is `path:line`, not prior task prose.
...
| **C5** | GAP | MED | Dual ADR location: `docs/adr/` uses 4-digit ids (`ADR-0001`, `ADR-0002`) while `.agents/documents/design/decisions/` uses 3-digit (`ADR-001`…`ADR-006`). Cross-citations jump between the two. | `docs/adr/ADR-0001-jsonwebtoken-10-msrv-bump.md:7` (cites `ADR-003`); `requirements/fsd.md:497,499` (cites `ADR-001`, `ADR-002`, `ADR-006`); `requirements/tdd.md:308,312` (cites `ADR-001`, `ADR-002`, `ADR-006`); `docs/adr/ADR-0002-...:73-75` acknowledges the split | **Open** — consolidate on one directory/numbering scheme (owner: next ADR editor). Tracked, not resolved by ADR-0002. |
...
| **C9** | STALE | LOW | README cites queue commit `e7e7e12`; milestones cites merged tree `7807ae4`. | `README.md:155` vs `docs/milestones.md:29` | **Fixed in `README.md`** — pin aligned to `7807ae4`. |
...
3. **C1/C2/C3/C9/C11 — README drift (HIGH/MED).** Stale status, wrong CLI name, and
   wrong crate list actively mislead readers. **Resolved in this task** for `README.md`.
4. **C14 — `rustasea-container` phantom crate (MED).** Three canonical docs claim a
   crate that does not exist; only `README.md` is corrected here.
5. **C5/C6/C7/C15/C16/C17 — doc lifecycle drift (MED/LOW).** Draft/Final markers,
   ADR locations, and stale counts erode trust in the doc tree. **Open.**
...
|---|---|---|---|
| C1, C2, C3, C4, C9, C11 | README sync | TASK-003 (this task) | Done |
| C8, C10 | Implement starter kit (SK-0…SK-6) + `cargo rustasea new` | engineering | M5/M6 |
| C14 | Remove `rustasea-container` from `manifest.md:11`, `application/README.md:44`, `fsd.md:497`; add container note to BC-0 | docs | next doc touch |
| C5 | Consolidate ADR location (`docs/adr/` 4-digit vs `design/decisions/` 3-digit) | tech lead | before next ADR |
...
| C13 | Derive crate counts from `Cargo.toml`; fix `presentation-brief.md:17`, `blueprint-audit.md:118`, `application/README.md:40` | docs | next doc touch |
...
- [`application/modules/manifest.md`](modules/manifest.md) — module index (contains C13/C14 drift).
