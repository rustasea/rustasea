...
|-----|-------|--------|-----------|----------|
| ADR-001 | HTTP framework: `axum` over `actix-web` | Accepted | M1 | `tower`-native, `tokio`-aligned, simpler ownership; see `decisions/ADR-001-axum-vs-actix.md` |
| ADR-002 | ORM: `sqlx` primary, `sea-orm` optional | Accepted | M2 | Compile-time checked queries + `pgvector`; `sea-orm` as shim behind feature flag; see `ADR-002-sqlx-vs-sea-orm.md` |
| ADR-003 | Async stack: single `tokio` runtime | Accepted | M0 | De-facto runtime powering `axum`/`sqlx`/`deadpool`; `tokio::select!` for shutdown; see `ADR-003-tokio-stack.md` |
| ADR-004 | Workspace crate boundaries per FR domain | Accepted | M0 | One crate per milestone domain; re-export via umbrella; see `ADR-004-workspace-crates.md` (scaffolded) |
| ADR-005 | Facades replaced by `AppState: Arc` | Accepted | M0 | `axum::extract::State` + `OnceLock`; no `static mut`; see ADR-003 §Alternatives |
| ADR-006 | Vector as feature-flagged Postgres extension | Accepted | M2/M6 | `pgvector` behind `vector` feature; MariaDB as second flag; see ADR-002 |

Full ADRs: `decisions/ADR-00*.md`. New cross-crate decisions **MUST** add an ADR and update this table.
...
| Workspace per-crate features | Incremental adoption, thin dep trees (`cargo tree` audit) | 18 crates increase publish/CI complexity | `cargo xtask ci` + single `Cargo.toml` workspace dependencies |
