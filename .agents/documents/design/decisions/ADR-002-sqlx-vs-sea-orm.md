...
|--------|------|------|---------|
| **sqlx primary + sea-orm shim** | Compile-time checked queries; proven `pgvector` and `deadpool` integration; minimal magic; single migration source of truth | More proc-macro work to reach Eloquent-level DX (offset by `rustavel-macros`) | **Chosen** — best balance of safety, vector story, and operator flexibility |
...
- Default `cargo check -p rustavel-orm` does **not** pull `sea-orm`; shim users add `features = ["sea-orm"]`.
- `rustavel-orm` crate exposes both `QueryBuilder<T>` (builder) and re-exported `sea_orm` entities when flagged — never both required simultaneously.
...
- `cargo tree --features sea-orm -p rustavel-orm` shows both, behind the same pool types.
