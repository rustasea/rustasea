...
//!
//! Listens for a domain event; `rustavel::events::Listener` declares an
...
//!
//! Listens for `rustavel::events::JobAttempted` by default; swap the event
//! type and register in `bootstrap/providers.rs`.

use rustavel::events::{{async_trait, EventError, JobAttempted, Listener}};
