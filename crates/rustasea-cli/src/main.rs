...
//! used. Errors map onto stable exit codes (see [`rustasea_cli::exit_code`]).
//!
...
use clap::Parser;
use rustasea_cli::{cli, load_default_commands, CliCommand, Command};
    ...
    runtime.block_on(async move {
        let result = match cli.command {
            CliCommand::List { json, all } => {
                let mut io = rustasea_cli::CommandOutput::default();
                ...
                let outcome = rustasea_cli::commands::builtins::List
                    .run(args, &mut io)
                    .await;
                outcome.map(|()| io)
            }
            CliCommand::Run(args) => {
                ...
                rustasea_cli::Artisan::call(signature, forwarded).await
            }
        };
        ...
        match result {
            ...
            Err(err) => {
                let code = rustasea_cli::exit_code(&err);
