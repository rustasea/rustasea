...
//!
//! The scaffold implements the `rustasea::cli::Command` trait. A generated
//! app crate depends on `rustasea` only (README M5 layout), so the
//! `#[async_trait]` expansion is imported from the umbrella crate's re-export
//! (`rustasea::cli::async_trait`) instead of the raw `async_trait` crate.
...
use rustasea::cli::async_trait::async_trait;
use rustasea::cli::error::CliResult;
use rustasea::cli::{{Command, Io}};
