...
fn route_macro_emits_metadata() {
    // The macro emits doc-hidden `__RUSTAVEL_ROUTE_<Fn>` consts in the
    // same module; they carry the parsed (method, path) pair.
    assert_eq!(__RUSTAVEL_ROUTE_index, ("GET", "/users"));
    assert_eq!(__RUSTAVEL_ROUTE_store, ("POST", "/users"));
    // `method` is optional and defaults to GET.
    assert_eq!(__RUSTAVEL_ROUTE_defaults, ("GET", "/defaults"));
}
