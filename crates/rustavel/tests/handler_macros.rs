...
//! metadata consts: `__RUSTAVEL_MIDDLEWARE_<Fn>` carries the middleware name
//! list and `__RUSTAVEL_AUTHORIZE_<Fn>` carries the `(ability, target)` pair.
...
use rustavel::macros::{authorize, middleware};
...
fn middleware_macro_emits_name_list() {
    assert_eq!(__RUSTAVEL_MIDDLEWARE_profile, &["auth:jwt"][..]);
    assert_eq!(
        __RUSTAVEL_MIDDLEWARE_login,
...
fn authorize_macro_emits_ability_and_target() {
    assert_eq!(__RUSTAVEL_AUTHORIZE_update_user, ("update", "User"));
    assert_eq!(__RUSTAVEL_AUTHORIZE_view_user, ("view", ""));
}
