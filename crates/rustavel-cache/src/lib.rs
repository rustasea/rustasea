...
//! default `Ok(false)` so custom drivers keep compiling, a moka-like in-memory
//! store, a redis wiring stub (no real Redis required this sprint), store
//! isolation (`redis` vs `memory`), and an atomic `Lock` with `get`/`block`/
//! `release`.
...
pub use repository::{CacheManager, Repository, StoreRef, MEMORY_STORE, REDIS_STORE};
