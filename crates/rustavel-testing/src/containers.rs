...
//! the container under a `rustavel-test-*` name so the shared teardown
//! removes it after the suite (FR-507, US-M5-04).
//!
//! Feature `containers` (default) gates this module's real docker wiring.
//! The image is configurable via `RUSTAVEL_TEST_PG_IMAGE` /
//! `RUSTAVEL_TEST_REDIS_IMAGE` so CI can reuse warm images.
...
pub struct ContainerHandle {
    /// Container name (`rustavel-test-<binary>-<suffix>`).
    pub name: String,
...
pub async fn postgres_container(suffix: &str) -> Result<ContainerHandle, ContainerError> {
    let image = std::env::var("RUSTAVEL_TEST_PG_IMAGE").unwrap_or_else(|_| PG_IMAGE.into());
    let port = allocate_port()?;
    let name = format!("rustavel-test-{}-{suffix}", std::process::id());
    ...
    Ok(ContainerHandle {
        name,
        port,
        url: format!("postgres://postgres:postgres@127.0.0.1:{port}/rustavel_test"),
    })
}
...
pub async fn redis_container(suffix: &str) -> Result<ContainerHandle, ContainerError> {
    let image = std::env::var("RUSTAVEL_TEST_REDIS_IMAGE").unwrap_or_else(|_| REDIS_IMAGE.into());
    let port = allocate_port()?;
    let name = format!("rustavel-test-{}-{suffix}", std::process::id());
...
/// Resolve the docker binary name (override via `RUSTAVEL_TEST_DOCKER`).
fn testcontainers_binary() -> String {
    std::env::var("RUSTAVEL_TEST_DOCKER").unwrap_or_else(|_| "docker".into())
}
