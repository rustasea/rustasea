...
//!
//! The derive lives in `rustavel-macros`; these integration tests exercise it
//! against real structs through the public `rustavel_orm` surface so the
//! emitted `Model` impl (table naming, timestamps, soft deletes, touch)
//! compiles and behaves end to end.

use chrono::{DateTime, Utc};
use rustavel_orm::{Model, SoftDeletes, Timestamps};
use uuid::Uuid;

/// Canonical derived model — snake_plural `users`, tracked timestamps and
/// soft deletes inferred from the field layout.
#[allow(dead_code)]
#[derive(rustavel_macros::Model)]
struct User {
    id: Uuid,
    name: String,
